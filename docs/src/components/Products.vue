...
export default {
	...
	methods: {
				...
				switch(this.page) {
					case 'cards':
						this.cards = this.params 
							...
							break;
					case 'decks':
						this.cards = await this.$store.dispatch('save_get_decks', this.last_card);
						break;
				}
				/* this.cards = this.params 
						? await this.$store.dispatch('get_cards_params', this.params.toLowerCase().split(' '), this.last_card)
		...
		set_init() {
			const actions = {
				...
				decks: () => {
					if(this.decks_pages.length && !this.params) {
						this.cards = [...this.decks_pages[0]];
						this.cards_save = [...this.decks_pages];
					}else {
						this.get_cards();
					}
				},
	...
	computed: {
		...
		decks_pages() {
            return this.$store.getters.decks_pages;
        },
